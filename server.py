...
class ArbitrageServerHandler(SimpleHTTPRequestHandler):
    ...
    hourly_reporter: Optional[HourlyFundingReporter] = None
    cache: Dict[str, Any] = {}
    ...
    def do_GET(self):
        ...
        elif path == "/api/telegram/hourly-report":
            self.handle_api_telegram_hourly_report(parsed.query)
        elif path == "/api/telegram/hourly-report/status":
    ...
    def do_POST(self):
        ...
        if path == "/api/telegram/test":
            self.handle_api_telegram_test()
        elif path in ["/api/telegram/hourly-report/send", "/api/telegram/hourly-report/trigger"]:
            self.handle_api_telegram_hourly_send()
    ...
    def handle_api_telegram_hourly_report(self, query_str: str = ""):
        if not self.hourly_reporter:
            payload = {
                "status": "error",
                "message": "Hourly funding reporter not initialized"
            }
            status_code = 500
        else:
            try:
                report_data = self.hourly_reporter.build_report()
                payload = {
                    "status": "success",
                    "data": report_data
                }
                ...
                payload = {
                    "status": "error",
                    "message": f"Failed to build hourly report: {str(e)}"
                }
                status_code = 500
    ...
    def handle_api_telegram_hourly_status(self):
        if self.hourly_reporter:
            status_data = self.hourly_reporter.get_status()
        else:
            status_data = {
                "enabled": False,
                "is_configured": False,
    ...
    def handle_api_telegram_hourly_send(self):
        if not self.hourly_reporter or not self.hourly_reporter.notifier.is_configured():
            payload = {
                "status": "error",
                "message": "Telegram credentials not configured or hourly reporter not initialized"
            }
            status_code = 400
        else:
            success, msg = self.hourly_reporter.send_report(force=True)
            payload = {
                "status": "success" if success else "error",
                "result": msg
            }
...
def start_web_server(port: int, 
    ...
    ArbitrageServerHandler.hourly_reporter = hourly_reporter
    ...
    if hourly_reporter and hourly_reporter.is_enabled():
        m_str = "包含全市场机会" if hourly_reporter.show_market else "仅账户实盘"
        print(f"💰 每小时资金费收益播报: 已启用 (每小时第 {hourly_reporter.report_minute} 分钟整点播报 | {m_str})")
        print(f"   ├─ 账户地址: {hourly_reporter.account_address or '未配置'}")
        print(f"   └─ 播报标的: {', '.join(hourly_reporter.symbols)}")
    else:
        print(f"💰 每小时资金费收益播报: 未启用 (可通过 .env 中 TELEGRAM_HOURLY_REPORT_ENABLED=true 启用)")
...
def main():
    ...
    # Hourly Funding Rate Report CLI overrides
    parser.add_argument("--hourly-report", action="store_true", default=None, help="Force enable hourly funding rate broadcast to Telegram")
    parser.add_argument("--no-hourly-report", action="store_true", default=False, help="Force disable hourly funding rate broadcast to Telegram")
    parser.add_argument("--hourly-report-minute", type=int, default=None, help="Minute of the hour to broadcast (default: 1)")
    parser.add_argument("--hourly-report-account", type=str, default=None, help="Master account address to monitor funding income for")
    ...
    # Resolve Hourly Funding Report settings
    hourly_enabled_override = None
    if args.no_hourly_report:
        hourly_enabled_override = False
    elif args.hourly_report:
        hourly_enabled_override = True
    ...
    hourly_reporter = HourlyFundingReporter(
        hl_client=hl_client,
        bybit_client=bybit_client,
        notifier=notifier,
        account_address=args.hourly_report_account,
        enabled=hourly_enabled_override,
        report_minute=args.hourly_report_minute,
    ...
    start_web_server(
        ...
        auto_engine=auto_engine,
        hourly_reporter=hourly_reporter
    )
